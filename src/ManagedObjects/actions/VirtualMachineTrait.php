...
use dnj\phpvmomi\DataObjects\ResourceAllocationInfo;
...
use dnj\phpvmomi\Exceptions\CreateVirtualMachineException;
use dnj\phpvmomi\Exceptions\CreateVirtualMachineTimeoutException;
...
trait VirtualMachineTrait
{
    ...
    {
        try {
            $vm = $this->api->getPropertyCollector()->_RetrieveProperties([
                'propSet' => [
                    'type' => 'VirtualMachine',
                    'all' => true,
                    'pathSet' => [
    ...
    public function transferISO(SplFileObject $file, string $isoDir = 'iso'): string
    {
        ...
        if (!$isoDatastore) {
            ...
            if ($isoDatastore->info->freeSpace <= $file->getSize()) {
                throw new Exception('can not find datastore with enough free space, datastore '.$isoDatastore->__toString()." does not have enough space! ({$file->getSize()} needed, currently {$isoDatastore->info->freeSpace} is free)");
            }
    ...
    {
        return ['VirtualE1000', 'VirtualSriovEthernetCard', 'VirtualVmxnet3'];
    }
    ...
    private static function createConfig(array $config)
    {
        ...
        $scsiController = null;
        if (false !== stripos($config['guest'], 'windows')) {
